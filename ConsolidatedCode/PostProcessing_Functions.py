...
def slice_up_time_series(signal, burn_in_time, end_time, run_count):# note that due to rounding error we cannot know the exact start and end time
    temp_signal = signal
    # temp_signal[:, 0] = signal[:, 0] - burn_in_time
    burn_in_time = 2
    end_time = 9
    new_start_time = np.where(temp_signal[:, 0] > burn_in_time)[0][0]
    print("start")

    new_end_time = np.where(  temp_signal[:, 0]  < end_time )[0]
    new_end_time = new_end_time.shape[0]
    new_start_state = new_start_state2 = np.zeros(temp_signal.shape[1])

    new_start_state2[1:] = temp_signal[new_start_time - 1, 1:]  #DMI, I am not sure we need the final colon
    #print(new_start_state2)
    print("start")
    print(new_start_time)
    print(new_end_time)
    #temp_signal[new_start_time - 1, :] = new_start_state
    sliced_up_signal = np.array(temp_signal[(new_start_time - 1):(new_end_time), :])#Take the value before new_start_time for the initial state

    print("end")
    print(sliced_up_signal[1,1])
    return sliced_up_signal
...
def compute_optimal_time_window_WhileLoop(signal, splits_matrix_into=1):
    ...
    if splits_matrix_into == 1:
        splits_matrix_into = 2 + (
                    np.shape(signal)[0] >> 11)  # this bit shifts to divide by 8192, then adds 2 avg 8192 rows per chunk
    ...
    for ixx in range(start_row, end_row):  # make first chunk
        lms[ixx - start_row, :] = np.float16(
            np.array(list(map(is_max_in_window(signal, (end_row - start_row), ixx + 1), range(nsample)))))
...
def all_together_now_Binom(signal, burn_in_time, rate1=0, number_of_samples1=0, rate2=1, number_of_samples2=0):
    ...
    uniformized = uniformly_sample(burn_in_time_series(signal, burn_in_time), rate=rate1,
                                   number_of_samples=number_of_samples1)
...
def all_together_nowUni(signal, burn_in_time, rate1=0, number_of_samples1=0, rate2=1,
                        number_of_samples2=0):  # short Run
...
def cleanStatsHeatMap(file_names, burn_in_time, rate1, weights=0):
    ...
    if weights == 0:
        stats = all_together_now_Once(signal=np.genfromtxt(file_names, delimiter=','), burn_in_time=burn_in_time,
                                      rate1=rate1, )
    ...
    if weights == 1:
        stats = all_together_nowUni(signal=np.genfromtxt(file_names, delimiter=','), burn_in_time=burn_in_time,
                                    rate1=rate1, )
        heat_map_matrices[:, 1, cv_axis] = stats
        mean_period = pd.DataFrame(heat_map_matrices[0, :, :])
        mean_amplitude = pd.DataFrame(heat_map_matrices[1, :, :])
    ...
    if weights == 2:
        stats = Fun.all_together_now_Triang(signal=np.genfromtxt(file_names, delimiter=','), burn_in_time=burn_in_time,
                                            rate1=rate1, )
        heat_map_matrices[:, 1, cv_axis] = stats
        mean_period = pd.DataFrame(heat_map_matrices[0, :, :])
        mean_amplitude = pd.DataFrame(heat_map_matrices[1, :, :])
    ...
    if weights == 3:
        stats = Fun.all_together_now_Binom(signal=np.genfromtxt(file_names, delimiter=','), burn_in_time=burn_in_time,
                                           rate1=rate1, )
        heat_map_matrices[:, 1, cv_axis] = stats
        mean_period = pd.DataFrame(heat_map_matrices[0, :, :])
        mean_amplitude = pd.DataFrame(heat_map_matrices[1, :, :])
