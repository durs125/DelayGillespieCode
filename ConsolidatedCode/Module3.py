...
def run_pipeline_splits(initialize_Gillespie, reactionList, runVector, path1):
    # Prepare the reactions classes.
    [alpha, beta, yr, r0, c0, mu, cv, initialState] = initialize_Gillespie
    initialState = int (initialState)
    [length_run, split_Size, burnInTime, sampleRate] = runVector
    dilution = Gill.Reaction(np.array([-1], dtype=int), 0, 0, [0, beta, 1, 0], 1, [0])
    enzymatic_degradation = Gill.Reaction(np.array([-1], dtype=int), 0, 0, [0, yr, r0, 1], 1, [0])
    production = Gill.Reaction(np.array([1], dtype=int), 0, 1, [alpha, c0, 2], 0, [mu, mu * cv])
...
def run_pipeline_single(initialize_Gillespie,  runVector, path1):
    ...
    [alpha, beta, yr, r0, c0, mu, cv, initialState, run_count ] = initialize_Gillespie
    initialState = np.array([initialState], dtype=int)
    params = [alpha, beta, yr, r0, c0, mu, cv, int(initialState)]
    ...
    np.savetxt("temp_signal.csv", params, delimiter=",")
    np.savetxt("temp_signal.csv", tempSignal, delimiter=",")
    print(tempSignal)
